...
class Stock extends CI_Controller {
	...
	function __construct()
	{
		...
		check_not_login();
		//check_admin();
        $this->load->model(['item_m', 'supplier_m', 'stock_m', 'customer_m']);
    ...
    function process()
    {
        if (isset($_POST['in_add'])) {
            ...
            if ($row_item->stock < $this->input->post('qty')) {
                $this->session->set_flashdata('error', "Jumlah Qty melebihi stock barang");
    ...
    public function stock_out_add()
    {
        ...
        $data = ['item' => $item, 'customer' => $customer, 'row' => $item1];
