...
class Stock_m extends CI_Model {
    ...
    {
        $this->db->select('t_stock.stock_id, p_item.barcode, p_item.name as item_name, qty, tanggal, 
        detail, supplier.name as supplier, p_item.item_id');
        $this->db->from('t_stock');
    ...
    {
        $this->db->select('t_stock.stock_id, p_item.barcode, p_item.name as item_name, qty, tanggal, 
        detail, customer.name as customer, p_item.item_id');
        $this->db->from('t_stock');
        $this->db->join('p_item', 't_stock.item_id = p_item.item_id');
        $this->db->join('customer', 't_stock.customer_id = customer.customer_id', 'left');
